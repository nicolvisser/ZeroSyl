...
def eval_boundaries(
    ...
    for i_utterance in range(len(seg)):
        ...
        if len(prediction) > 0 and len(ground_truth) > 0 and abs(prediction[-1] - ground_truth[-1]) <= tolerance:
...
def eval_token_boundaries(
    ...
    for i_utterance in range(len(seg)):
        ...
        for seg_start, seg_end in seg_segments:
            for i_segment, (ref_start_interval, ref_end_interval) in enumerate(ref_intervals):
