...
acoustic = AcousticModel.from_pretrained_checkpoint(
    "wandb/run-20251125_213214-aeefkozv/files/step-20000.pt"
).cuda()
wavtokenizer = load_wavtokenizer_small_600_24k_4096().cuda()

segments_dir = Path("output/segments/syllablelm-625-official-ids-k-8192/LibriSpeech")
...
for segments_path in segments_paths:
    ...
    AudioEncoder(audio_out, sample_rate=24000).to_file(
        f"output/synth/{segments_path.stem}-syllablelm.wav",
        sample_rate=24000,
        num_channels=1,
    )
